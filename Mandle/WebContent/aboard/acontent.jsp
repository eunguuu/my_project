...
<table id="content">
  <tr><td class="h">글번호</td><td class="d"><%=bb.getNum() %></td>
   ...
   <%if(bb.getFile() != null) {
		%><a href="../upload/<%=bb.getFile() %>" class="a1"><%=bb.getFile() %></a><br>
		<a href="file_down.jsp?file_name=<%=bb.getFile() %>" class="a2">
   		첨부파일다운 : <%=bb.getFile() %></a><br><%
   } else {%>첨부파일 없음<%}%></td></tr>
...
<input type="button" value="글수정" class="btn" onclick="location.href='aupdateForm.jsp?num=<%=bb.getNum() %>'">
<input type="button" value="글삭제" class="btn" onclick="location.href='adeleteForm.jsp?num=<%=bb.getNum() %>'">
